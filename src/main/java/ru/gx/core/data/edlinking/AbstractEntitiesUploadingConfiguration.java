...
public abstract class AbstractEntitiesUploadingConfiguration implements EntitiesUploadingConfiguration {
    ...
    EntityUploadingDescriptor<CH, E, O> getByChannel(
            @NotNull final CH channelHandleDescriptor
    ) throws EntitiesDtoLinksConfigurationException {
        final var result = this.descriptorsByChannels.get(channelHandleDescriptor);
        if (result == null) {
            throw new EntitiesDtoLinksConfigurationException("Can't get descriptor by channel " + channelHandleDescriptor.getClass().getName());
        }
    ...
    EntityUploadingDescriptor<CH, E, O> newDescriptor(
        ...
        if (this.descriptorsByChannels.containsKey(channelApiDescriptor)) {
            throw new EntitiesDtoLinksConfigurationException("channelHandleDescriptor " + channelApiDescriptor.getClass().getName() + " already registered!");
        }
